...
function CylindericalNum({ glassMinusRange, data, store }) {
  ...
  useEffect(() => {
    ...
    setDataSource(rows);
    // setDataSource(rows.filter((item) => Object.keys(item).length));
    // } else {
    //   setDataSource([]);
    // }
          ...
          <Form.Item>
            ...
            {dataSource.map((item) => {
              return item.isDivider ? (
                ...
                <Row key={item.key} gutter={16} style={{ paddingBottom: 5 }}>
                  <Col span={8}>
                    <Row>
                      <Col>
